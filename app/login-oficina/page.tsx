...
import { AlertCircle, Loader2, Mail, Lock, Wrench, LogIn, CheckCircle } from "lucide-react";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (profile && !authLoading && profile.type === "oficina") {
        ...
        <div className="max-w-6xl w-full">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            ...
            <div className="hidden lg:flex flex-col items-center justify-center bg-gradient-to-br from-yellow-500 to-orange-600 rounded-3xl p-12 text-white shadow-2xl backdrop-blur-xl">
              ...
              <p className="text-yellow-100 text-center text-lg leading-relaxed mb-6">
                Acesse seu sistema de gestão e continue gerenciando sua oficina com eficiência.
              </p>
              <div className="space-y-3 w-full max-w-md">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-yellow-50">Gestão completa de OS</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center flex-shrink-0">
                    <CheckCircle className="h-5 w-5" />
                  </div>
                  <span className="text-yellow-50">Controle financeiro</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center flex-shrink-0">
                    <CheckCircle className="h-5 w-5" />
                  </div>
                  <span className="text-yellow-50">Relatórios em tempo real</span>
                </div>
              </div>
            </div>
              ...
              <Card className="border-2 shadow-2xl">
                <CardHeader className="space-y-1 pb-6">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <CardTitle className="text-3xl font-heading font-bold">Entrar</CardTitle>
                  </div>
                  <CardDescription className="text-base">
                    Acesse sua conta de oficina
                  </CardDescription>
                </CardHeader>
                <form onSubmit={handleSubmit}>
                  <CardContent className="space-y-5">
                    ...
                    <Button
                      type="submit"
                      className="w-full h-12 text-base font-bold bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-500 hover:to-yellow-600 text-gray-900 shadow-lg hover:shadow-xl transition-all"
                      disabled={loading}
                    >
                      {loading ? (
                        <>
                    ...
                    <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-4 text-center">
                      <p className="text-sm text-gray-700">
                        É um motorista?{" "}
                        <Link href="/login-motorista" className="font-bold text-blue-600 hover:text-blue-700 underline">
                          Entre aqui
                        </Link>
                      </p>
                    </div>
