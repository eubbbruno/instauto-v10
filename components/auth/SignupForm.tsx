"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { Car, Wrench, Mail, Lock, Eye, EyeOff, Loader2, CheckCircle, User } from "lucide-react";
import Link from "next/link";
import Image from "next/image";
import { toast } from "sonner";
import { createClient } from "@/lib/supabase";
import { FadeIn } from "@/components/ui/motion";

type UserType = "motorista" | "oficina";

const COPY: Record<UserType, {
  eyebrow: string;
  title: React.ReactNode;
  subtitle: string;
  nameLabel: string;
  namePlaceholder: string;
  features: { title: string; sub: string }[];
}> = {
  motorista: {
    eyebrow: "Cadastro de motorista",
    title: <>Cuide do seu carro<br /><span className="text-brand-yellow">sem dor de cabeça.</span></>,
    subtitle: "Encontre oficinas de confiança, peça orçamentos e organize as manutenções do seu veículo — tudo de graça.",
    nameLabel: "Seu Nome",
    namePlaceholder: "João Silva",
    features: [
      { title: "Orçamentos Grátis", sub: "Compare preços sem compromisso" },
      { title: "Oficinas Verificadas", sub: "Avaliações reais de motoristas" },
      { title: "Sua Garagem Digital", sub: "Histórico e lembretes num só lugar" },
    ],
  },
  oficina: {
    eyebrow: "Cadastro de oficina",
    title: <>Sua oficina no mapa<br /><span className="text-brand-yellow">de milhares.</span></>,
    subtitle: "Sistema completo de gestão + visibilidade para novos clientes. Comece com 14 dias de PRO grátis, sem cartão.",
    nameLabel: "Nome da Oficina",
    namePlaceholder: "Auto Center Silva",
    features: [
      { title: "14 dias de PRO grátis", sub: "Recursos completos, sem cartão" },
      { title: "Gestão Completa", sub: "Clientes, OS, estoque e financeiro" },
      { title: "Novos Clientes", sub: "Apareça para quem busca oficina" },
    ],
  },
};

export default function SignupForm({ userType }: { userType: UserType }) {
  const router = useRouter();
  const copy = COPY[userType];

  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [name, setName] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [loading, setLoading] = useState(false);
  const [googleLoading, setGoogleLoading] = useState(false);
  const [facebookLoading, setFacebookLoading] = useState(false);
  const [showEmailConfirmation, setShowEmailConfirmation] = useState(false);
  const [registeredEmail, setRegisteredEmail] = useState("");

  const supabase = createClient();

  const handleSignup = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);

    try {
      if (!name || name.trim().length < 2) {
        toast.error("Por favor, preencha seu nome");
        setLoading(false);
        return;
      }

      const profileType = userType === "oficina" ? "workshop" : "motorist";

      // Salvar tipo no cookie ANTES do signUp (para o callback ler depois)
      document.cookie = `instauto_user_type=${userType}; path=/; max-age=3600`;

      const { data, error } = await supabase.auth.signUp({
        email,
        password,
        options: {
          emailRedirectTo: `${window.location.origin}/auth/callback`,
          data: {
            name: name,
            user_type: profileType,
          },
        },
      });

      if (error) throw error;
      if (!data.user) throw new Error("Erro ao criar conta. Tente novamente.");

      // Sem session = precisa confirmar email
      if (!data.session) {
        setRegisteredEmail(email);
        setShowEmailConfirmation(true);
        setLoading(false);
        return;
      }

      // Com session: criar profile
      const { error: profileError } = await supabase.from("profiles").insert({
        id: data.user.id,
        email: data.user.email,
        name: name,
        type: profileType,
      });

      if (profileError) throw new Error("Erro ao criar perfil: " + profileError.message);

      if (profileType === "workshop") {
        const { error: workshopError } = await supabase.from("workshops").insert({
          profile_id: data.user.id,
          name: name || "Minha Oficina",
          plan_type: "free",
          subscription_status: "trial",
          // Trial reverso: 14 dias de PRO completo, sem cartão (ver lib/plan.ts TRIAL_DAYS)
          trial_ends_at: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString(),
          is_public: true,
          accepts_quotes: true,
        });
        if (workshopError) throw new Error("Erro ao criar oficina: " + workshopError.message);

        toast.success("Conta criada com sucesso!");
        setTimeout(() => router.push("/oficina"), 500);
      } else {
        const { error: motoristError } = await supabase.from("motorists").insert({
          profile_id: data.user.id,
        });
        if (motoristError) throw new Error("Erro ao criar motorista: " + motoristError.message);

        toast.success("Conta criada com sucesso!");
        setTimeout(() => router.push("/motorista"), 500);
      }
    } catch (error: any) {
      console.error("❌ [Cadastro] Erro:", error);
      toast.error(error.message || "Erro ao criar conta");
    } finally {
      setLoading(false);
    }
  };

  const handleOAuthSignup = async (provider: "google" | "facebook") => {
    const setBusy = provider === "google" ? setGoogleLoading : setFacebookLoading;
    setBusy(true);
    try {
      // Grava o tipo escolhido para o callback criar o profile correto
      localStorage.setItem("instauto_user_type", userType);
      document.cookie = `instauto_user_type=${userType}; path=/; max-age=3600`;

      const { error } = await supabase.auth.signInWithOAuth({
        provider,
        options: {
          redirectTo: `${window.location.origin}/auth/callback`,
        },
      });

      if (error) throw error;
    } catch (error: any) {
      console.error(`❌ [${provider}] Erro:`, error);
      toast.error(`Erro ao conectar com ${provider === "google" ? "Google" : "Facebook"}`);
      setBusy(false);
    }
  };

  const otherType = userType === "oficina" ? "motorista" : "oficina";

  return (
    <div className="min-h-screen flex">
      {/* Lado Esquerdo - Branding (Desktop) */}
      <div className="hidden lg:flex lg:w-1/2 bg-navy text-white p-12 flex-col justify-center relative overflow-hidden">
        <div className="absolute -top-40 -right-20 w-[500px] h-[500px] rounded-full bg-brand-blue/20 blur-[100px] pointer-events-none" />
        <div className="absolute -bottom-40 -left-20 w-[400px] h-[400px] rounded-full bg-brand-yellow/8 blur-[80px] pointer-events-none" />

        <div className="relative z-10">
          <Link href="/" className="inline-block mb-14">
            <Image
              src="/images/instauto-amarelo-branco.svg"
              alt="Instauto"
              width={160}
              height={44}
              className="h-10 w-auto"
            />
          </Link>

          <p className="text-eyebrow text-brand-gold mb-4">{copy.eyebrow}</p>
          <h1 className="font-heading text-4xl lg:text-5xl font-black leading-tight tracking-tight mb-6">
            {copy.title}
          </h1>

          <p className="text-white/55 text-lg mb-12 max-w-sm leading-relaxed">
            {copy.subtitle}
          </p>

          <div className="space-y-5">
            {copy.features.map(({ title, sub }) => (
              <div key={title} className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-brand-yellow/15 ring-1 ring-brand-yellow/25 flex items-center justify-center flex-shrink-0">
                  <CheckCircle className="w-5 h-5 text-brand-yellow" />
                </div>
                <div>
                  <p className="font-semibold text-white text-[15px]">{title}</p>
                  <p className="text-white/45 text-sm">{sub}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>

      {/* Lado Direito - Formulário */}
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 bg-[#F8F9FB]">
        <FadeIn className="w-full max-w-md">
          <div className="w-full max-w-md">
            {/* Logo Mobile */}
            <div className="lg:hidden text-center mb-8">
              <Link href="/">
                <div className="inline-flex items-center gap-2 bg-navy px-4 py-2 rounded-xl">
                  <Image src="/images/instauto-amarelo-branco.svg" alt="Instauto" width={120} height={32} className="h-7 w-auto" />
                </div>
              </Link>
            </div>

            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8">
              {showEmailConfirmation ? (
                <div className="text-center py-4">
                  <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6">
                    <Mail className="w-10 h-10 text-green-600" />
                  </div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3">
                    Verifique seu email! 📧
                  </h2>
                  <p className="text-gray-600 mb-3 text-sm sm:text-base">
                    Enviamos um link de confirmação para:
                  </p>
                  <p className="font-semibold text-blue-600 mb-6 text-sm sm:text-base break-all px-4">
                    {registeredEmail}
                  </p>
                  <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4 mb-6">
                    <p className="text-sm text-blue-900 font-medium mb-2">📬 Próximos passos:</p>
                    <ol className="text-sm text-blue-800 text-left space-y-2">
                      <li>1. Abra seu email</li>
                      <li>2. Clique no link de confirmação</li>
                      <li>3. Você será redirecionado automaticamente</li>
                    </ol>
                  </div>
                  <Link
                    href="/login"
                    className="block w-full py-3 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition-colors"
                  >
                    Entendi, vou verificar
                  </Link>
                  <p className="text-xs text-gray-400 mt-3">
                    Não recebeu? Verifique a pasta de spam ou tente novamente.
                  </p>
                </div>
              ) : (
                <>
                  {/* Badge do tipo escolhido */}
                  <div className="flex justify-center mb-4">
                    <span className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-bold ${
                      userType === "oficina"
                        ? "bg-brand-yellow/15 text-navy ring-1 ring-brand-yellow/30"
                        : "bg-navy/5 text-navy ring-1 ring-navy/15"
                    }`}>
                      {userType === "oficina" ? <Wrench className="w-4 h-4" /> : <Car className="w-4 h-4" />}
                      {userType === "oficina" ? "Oficina" : "Motorista"}
                    </span>
                  </div>

                  <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 text-center mb-2">
                    Criar conta
                  </h1>
                  <p className="text-sm sm:text-base text-gray-500 text-center mb-6">
                    Comece a usar o Instauto agora
                  </p>

                  {/* Botão Google */}
                  <button
                    onClick={() => handleOAuthSignup("google")}
                    disabled={googleLoading || facebookLoading}
                    className="w-full flex items-center justify-center gap-3 px-4 py-3.5 sm:py-3 bg-white border-2 border-gray-200 rounded-xl hover:bg-gray-50 transition-colors mb-3 disabled:opacity-50 min-h-[48px]"
                  >
                    {googleLoading ? (
                      <Loader2 className="w-5 h-5 animate-spin" />
                    ) : (
                      <svg className="w-5 h-5" viewBox="0 0 24 24">
                        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                      </svg>
                    )}
                    <span className="font-medium text-gray-700">
                      {googleLoading ? "Conectando..." : "Continuar com Google"}
                    </span>
                  </button>

                  {/* Botão Facebook */}
                  <button
                    onClick={() => handleOAuthSignup("facebook")}
                    disabled={googleLoading || facebookLoading}
                    className="w-full flex items-center justify-center gap-3 px-4 py-3.5 sm:py-3 bg-[#1877F2] rounded-xl hover:bg-[#166FE5] transition-colors mb-4 disabled:opacity-50 min-h-[48px]"
                  >
                    {facebookLoading ? (
                      <Loader2 className="w-5 h-5 animate-spin text-white" />
                    ) : (
                      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="#FFFFFF">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                      </svg>
                    )}
                    <span className="font-medium text-white">
                      {facebookLoading ? "Conectando..." : "Continuar com Facebook"}
                    </span>
                  </button>

                  <div className="flex items-center gap-4 my-6">
                    <div className="flex-1 h-px bg-gray-200" />
                    <span className="text-sm text-gray-400">ou</span>
                    <div className="flex-1 h-px bg-gray-200" />
                  </div>

                  <form onSubmit={handleSignup} className="space-y-4">
                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-2 block">
                        {copy.nameLabel} *
                      </label>
                      <div className="relative">
                        <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                        <input
                          type="text"
                          value={name}
                          onChange={(e) => setName(e.target.value)}
                          placeholder={copy.namePlaceholder}
                          required
                          minLength={2}
                          className="w-full pl-12 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                        />
                      </div>
                    </div>

                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-2 block">Email *</label>
                      <div className="relative">
                        <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                        <input
                          type="email"
                          value={email}
                          onChange={(e) => setEmail(e.target.value)}
                          placeholder="seu@email.com"
                          required
                          className="w-full pl-12 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                        />
                      </div>
                    </div>

                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-2 block">Senha *</label>
                      <div className="relative">
                        <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                        <input
                          type={showPassword ? "text" : "password"}
                          value={password}
                          onChange={(e) => setPassword(e.target.value)}
                          placeholder="••••••••"
                          required
                          minLength={6}
                          className="w-full pl-12 pr-12 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                        />
                        <button
                          type="button"
                          onClick={() => setShowPassword(!showPassword)}
                          className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors"
                        >
                          {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                        </button>
                      </div>
                    </div>

                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full btn-epic-blue py-4 rounded-xl font-bold disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-6 min-h-[48px]"
                    >
                      {loading && <Loader2 className="w-5 h-5 animate-spin" />}
                      Criar conta
                    </button>
                  </form>

                  <div className="mt-6 text-center space-y-2">
                    <p className="text-sm text-gray-600">
                      Já tem conta?{" "}
                      <Link href="/login" className="text-blue-600 font-semibold hover:text-blue-700 hover:underline transition-colors">
                        Fazer login
                      </Link>
                    </p>
                    <p className="text-sm text-gray-500">
                      Quer se cadastrar como {otherType === "oficina" ? "oficina" : "motorista"}?{" "}
                      <Link href={`/cadastro/${otherType}`} className="text-blue-600 font-semibold hover:text-blue-700 hover:underline transition-colors">
                        Clique aqui
                      </Link>
                    </p>
                  </div>
                </>
              )}
            </div>

            <div className="text-center mt-8">
              <Link
                href="/"
                className="inline-flex items-center gap-2 text-gray-500 hover:text-gray-700 transition-colors text-sm font-medium"
              >
                <span>←</span>
                <span>Voltar para o site</span>
              </Link>
            </div>
          </div>
        </FadeIn>
      </div>
    </div>
  );
}
